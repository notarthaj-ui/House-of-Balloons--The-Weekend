...
function updateControls() { const playing = !audio.paused; toggle.textContent = playing ? 'Ⅱ' : '▶'; toggle.setAttribute('aria-label', playing ? 'Pause' : 'Play'); heroPlay.innerHTML = `<span>${playing ? 'Ⅱ' : '▶'}</span>${playing ? 'Pause current track' : 'Play the first chapter'}`; trackRows.forEach((row, index) => row.classList.toggle('is-active', index === activeTrack)); }
function loadTrack(index, autoplay = false) { activeTrack = (index + tracks.length) % tracks.length; audio.src = tracks[activeTrack][1]; nowPlaying.textContent = tracks[activeTrack][0]; seek.value = '0'; timeDisplay.textContent = '0:00 / --:--'; audio.load(); if (autoplay) audio.play().catch(updateControls); updateControls(); }
